...
const (
	...
	BifrostContextKeyClientRequestedModel                BifrostContextKey = "bifrost-client-requested-model"                // string (set by governance HTTP pre-hook) - the client-facing model BEFORE any routing rule rewrote it, so integrations can echo it back (e.g. Anthropic responses must report the requested model for thinking-block continuity)
