...
<Note>
Supports text and multimodal (text + image) embeddings. Multiple input items are sent as a batch — each item maps to a separate entry in `requests[]`.
</Note>

**Request Parameters**:
- `input` → each item in the `[]EmbeddingContent` array maps to a separate `requests[]` entry (batch). Each content's parts map to `content.parts[]`.
