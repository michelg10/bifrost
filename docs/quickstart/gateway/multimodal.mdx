...
Bifrost supports multimodal embeddings — generating vector representations that capture text, images, audio, video, and documents together. The HTTP `input` field accepts a string, an array of strings, or a full `EmbeddingContent` array for multimodal content.

### Text Embedding (all providers)

The simplest form — pass a string or array of strings directly in `input`:
...
curl -X POST http://localhost:8080/v1/embeddings \
  ...
  -d '{
    ...
    "input": ["a golden retriever playing fetch"],
...
For multimodal content, pass `input` as a `[]EmbeddingContent` array. Each item is an array of typed parts:
...
curl -X POST http://localhost:8080/v1/embeddings \
  ...
  -d '{
    ...
    "input": [
      [
        { "type": "text",  "text": "a golden retriever in a park" },
        { "type": "image", "image": { "url": "data:image/jpeg;base64,<base64-encoded-jpeg>" } }
      ]
    ],
    "input_type": "image"
  }'
...
Gemini supports text, image, audio, video, and PDF in the same content entry:
...
curl -X POST http://localhost:8080/v1/embeddings \
  ...
  -d '{
    ...
    "input": [
      [
        { "type": "text",  "text": "transcript of the audio" },
        { "type": "audio", "audio": { "url": "https://example.com/speech.mp3" } }
      ]
    ]
  }'
```

For PDFs, use the `file` part type:
...
`multimodalembedding@001` accepts text, image, and video parts. Images and videos must be GCS URIs (`gs://`) or base64 data URIs. The response contains a separate embedding per modality.

```bash
curl -X POST http://localhost:8080/v1/embeddings \
  -H "Content-Type: application/json" \
  -d '{
    "model": "vertex/multimodalembedding@001",
    "input": [
      [
        { "type": "text",  "text": "product photo" },
        { "type": "image", "image": { "url": "gs://my-bucket/photo.jpg" } }
      ]
    ],
...
**Response** contains separate vectors per modality:
...
For video, provide a `video` part with an optional segment config:
...
"input": [
  [
    {
      "type": "video",
      "video": { "url": "gs://my-bucket/clip.mp4" },
      "video_config": { "start_offset_sec": 0, "end_offset_sec": 30 }
    }
  ]
]
```
