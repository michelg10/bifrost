...
Bifrost supports multimodal embeddings — generating vector representations that capture text, images, audio, video, and documents together. The `Input` field on `BifrostEmbeddingRequest` is a `[]EmbeddingContent`, where each `EmbeddingContent` is a slice of typed parts.

### Text Embedding (all providers)

Pass a single-part text content. For providers that only need a string you can also pass `"input": "text"` via the HTTP API.
...
Cohere `embed-v4.0` accepts a text part and an image part in the same content entry. The image can be a base64 data URI or a direct URL.

```go
imageURL := "data:image/jpeg;base64,<base64-encoded-jpeg>"
...
resp, err := client.EmbeddingRequest(schemas.NewBifrostContext(ctx, schemas.NoDeadline), &schemas.BifrostEmbeddingRequest{
    ...
    Params: &schemas.EmbeddingParameters{
        ExtraParams: map[string]interface{}{"input_type": "image"},
    },
})
```
...
Gemini supports text, image, audio, video, and PDF in the same content entry. Pass each modality as a typed part:
...
resp, err := client.EmbeddingRequest(schemas.NewBifrostContext(ctx, schemas.NoDeadline), &schemas.BifrostEmbeddingRequest{
    ...
    Input: []schemas.EmbeddingContent{
        {
            {Type: schemas.EmbeddingContentPartTypeText,  Text:  &text},
            {Type: schemas.EmbeddingContentPartTypeAudio, Audio: &schemas.EmbeddingMediaPart{URL: &audioURL}},
        },
    },
})
```

For PDFs, use the `file` part type with `MIMEType: "application/pdf"`:
...
resp, err := client.EmbeddingRequest(schemas.NewBifrostContext(ctx, schemas.NoDeadline), &schemas.BifrostEmbeddingRequest{
    ...
    Input: []schemas.EmbeddingContent{
        {
            {Type: schemas.EmbeddingContentPartTypeFile, File: &schemas.EmbeddingMediaPart{URL: &pdfURL, MIMEType: &pdfMime}},
        },
    },
})
```

### Image + Text + Video Embedding

`multimodalembedding@001` accepts text, image, and video in a single content entry. Images and videos must be GCS URIs (`gs://`) or base64 data URIs. The response contains a separate `EmbeddingData` per modality.

```go
imageGCS := "gs://my-bucket/photo.jpg"
text     := "product photo"
resp, err := client.EmbeddingRequest(schemas.NewBifrostContext(ctx, schemas.NoDeadline), &schemas.BifrostEmbeddingRequest{
    ...
    Input: []schemas.EmbeddingContent{
        {
            {Type: schemas.EmbeddingContentPartTypeText,  Text:  &text},
            {Type: schemas.EmbeddingContentPartTypeImage, Image: &schemas.EmbeddingMediaPart{URL: &imageGCS}},
        },
    },
})
// Response has separate vectors per modality:
// resp.Data[0].Modality == "text"
// resp.Data[1].Modality == "image"
```
