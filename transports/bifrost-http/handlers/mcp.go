...
func (h *MCPHandler) addMCPClient(ctx *fasthttp.RequestCtx) {
	...
	// TODO: @ANUJ Why is client id being taken from the request? It should be generated by the server.
	if req.ClientID == "" {
	...
	// --- DB writes ---
	if isOAuth {
		// Single transaction: create MCP client (back-ref set) + create oauth_config (FK set).
		// Any failure rolls back both inserts atomically — no manual cleanup needed.
		if err := h.store.ConfigStore.ExecuteTransaction(ctx, func(tx *gorm.DB) error {
			if err := h.store.ConfigStore.CreateMCPClientConfig(ctx, newClientConfig, tx); err != nil {
				return fmt.Errorf("create MCP client: %w", err)
			}
			if err := tx.WithContext(ctx).Create(prepared.Record).Error; err != nil {
				return fmt.Errorf("create oauth config: %w", err)
			}
			return nil
		}); err != nil {
			SendError(ctx, fasthttp.StatusInternalServerError, fmt.Sprintf("Failed to create MCP client with OAuth config: %v", err))
			return
		}
...
func (h *MCPHandler) completeMCPClientOAuth(ctx *fasthttp.RequestCtx) {
	...
	// isUpdateFlow: client already in the runtime manager (OAuth re-auth on a live client).
	isUpdateFlow := false
	bfClients, err2 := h.client.GetMCPClients()
	if err2 != nil {
		logger.Error(fmt.Sprintf("[OAuth Complete] Failed to list runtime MCP clients; cannot safely determine flow type: %v", err2))
		SendError(ctx, fasthttp.StatusInternalServerError, fmt.Sprintf("failed to get MCP clients from Bifrost: %v", err2))
		return
	}
	for _, c := range bfClients {
		if c.Config.ID == mcpClientConfig.ID {
			isUpdateFlow = true
			break
		}
	}
	...
	if isUpdateFlow {
		...
		oldOauthConfigID := ""
		if existingDBConfig != nil && existingDBConfig.OauthConfigID != nil && *existingDBConfig.OauthConfigID != "" &&
		...
		if err := h.mcpManager.AddMCPClient(ctx, mcpClientConfig); err != nil {
			if existingDBConfig != nil {
				if rbErr := h.store.ConfigStore.UpdateMCPClientConfig(ctx, mcpClientConfig.ID, existingDBConfig); rbErr != nil {
					logger.Error(fmt.Sprintf("Failed to rollback MCP client DB update: %v. please restart bifrost to keep core and database in sync", rbErr))
				}
			}
