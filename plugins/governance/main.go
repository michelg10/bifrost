...
func (p *GovernancePlugin) HTTPTransportPreHook(ctx *schemas.BifrostContext, req *schemas.HTTPRequest) (*schemas.HTTPResponse, error) {
	...
	// WebSocket upgrades: GET /v1/realtime?model=...) or if large payload mode is active.
...
// (e.g. WebSocket upgrade GET /v1/realtime?model=...) where the model lives in a
// query parameter instead of the JSON body. We build a synthetic payload so routing
// rules and VK load-balancing can evaluate normally, then propagate any model
// rewrite back to the query param for the downstream handler to pick up.
func (p *GovernancePlugin) governRealtimeQueryParam(ctx *schemas.BifrostContext, req *schemas.HTTPRequest, virtualKeyValue *string, hasRoutingRules bool) (*schemas.HTTPResponse, error) {
	...
	// Propagate model changes back to the query param so the downstream
	// realtime handler sees the routed/load-balanced model.
	if newModel, ok := payload["model"].(string); ok && newModel != originalModel {
		req.Query["model"] = newModel
	}
